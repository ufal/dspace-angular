...
export class HandleTableComponent implements OnInit {
   ...
   */
  handlesRD$: BehaviorSubject<RemoteData<PaginatedList<Handle>>> =
    new BehaviorSubject<RemoteData<PaginatedList<Handle>>>(null);
   ...
   */
  getAllHandles() {
    this.handlesRD$ = new BehaviorSubject<RemoteData<PaginatedList<Handle>>>(
      null
    );
    this.isLoading = true;
    ...
    observableCombineLatest([currentPagination$, currentSort$])
      .pipe(
        ...
        handleRD.payload.page.forEach((handle) => {
          if (handle.id === this.selectedHandle) {
            ...
            this.router.navigate([this.handleRoute, this.editHandlePath], {
              queryParams: {
                id: handle.id,
                _selflink: handle._links.self.href,
                handle: handle.handle,
                url: handle.url,
                resourceType: handle.resourceTypeID,
  ...
  deleteHandles() {
    ...
    // check response
    this.requestService.getByUUID(requestId).subscribe((info) => {
      // if is empty
      if (
        !isNotEmpty(info) ||
        !isNotEmpty(info.response) ||
        !isNotEmpty(info.response.statusCode)
      ) {
      ...
      if (
        info.response.statusCode
          .toString()
          .startsWith(SUCCESSFUL_RESPONSE_START_CHAR)
      ) {
        this.notificationsService.success(
          null,
          this.translateService.get(
            'handle-table.delete-handle.notify.successful'
          )
        );
      } else {
        ...
        this.translateService
          .get('handle-table.delete-handle.notify.error')
          .pipe(take(1))
          .subscribe((message) => {
